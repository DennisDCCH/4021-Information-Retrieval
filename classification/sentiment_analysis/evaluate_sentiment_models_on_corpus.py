...
CORPUS_PREDICTIONS_PATH = os.path.join(RESULTS_DIR, 'mcu_imdb_reviews_preprocessed_predictions.csv')
CORPUS_SUMMARY_PATH = os.path.join(RESULTS_DIR, 'mcu_imdb_reviews_preprocessed_summary.csv')
...
def load_preprocessed_corpus() -> pd.DataFrame:
    if not os.path.exists(PREPROCESSED_PATH):
        raise FileNotFoundError(f'Preprocessed dataset not found at: {PREPROCESSED_PATH}')

    corpus_df = pd.read_csv(PREPROCESSED_PATH)
    corpus_df = ensure_review_id(corpus_df)

    if 'processed_text' not in corpus_df.columns:
        raise ValueError(f"Expected 'processed_text' column in {PREPROCESSED_PATH}")

    corpus_df = corpus_df.rename(columns={'processed_text': 'preprocessed_text'})
    corpus_df['preprocessed_text'] = corpus_df['preprocessed_text'].fillna('')
    return corpus_df
...
def evaluate_preprocessed_models_on_corpus(
    trained_artifacts: Dict[str, Dict[str, object]],
) -> None:
    corpus_df = load_preprocessed_corpus().copy()
    x_corpus = corpus_df['preprocessed_text'].fillna('')

    prediction_rows: List[pd.DataFrame] = []
    summary_rows: List[Dict[str, object]] = []

    for spec in MODEL_SPECS:
        if spec.kind not in {'classical', 'transformer'}:
            continue

        artifact = trained_artifacts.get(spec.name)
        if artifact is None:
            raise ValueError(f'Missing trained artifact for model: {spec.name}')

        start_time = time.perf_counter()
        if artifact.get('kind') == 'transformer':
            tokenizer = artifact['tokenizer']
            trainer = artifact['trainer']
            corpus_dataset = EncodedTextDataset(
                x_corpus,
                pd.Series([0] * len(corpus_df), index=corpus_df.index),
                tokenizer,
            )
            prediction_output = trainer.predict(corpus_dataset)
            logits_tensor = torch.tensor(prediction_output.predictions)
            y_pred = pd.Series(torch.argmax(logits_tensor, dim=1).numpy().astype(int), index=corpus_df.index)
            prob_pos = pd.Series(torch.softmax(logits_tensor, dim=1)[:, 1].numpy(), index=corpus_df.index)
        else:
            vectorizer = artifact.get('vectorizer')
            model = artifact.get('model')
            if vectorizer is None or model is None:
                raise ValueError(f'Missing in-memory classical artifact for model: {spec.name}')

            x_corpus_vec = vectorizer.transform(x_corpus)
            y_pred = pd.Series(model.predict(x_corpus_vec), index=corpus_df.index)
            if hasattr(model, 'predict_proba'):
                prob_pos = pd.Series(model.predict_proba(x_corpus_vec)[:, 1], index=corpus_df.index)
            else:
                prob_pos = pd.Series([pd.NA] * len(corpus_df), index=corpus_df.index)

        classification_time = time.perf_counter() - start_time
        predicted_positive = int((y_pred == 1).sum())
        predicted_negative = int((y_pred == 0).sum())

        model_predictions = corpus_df.copy()
        model_predictions['model'] = spec.name
        model_predictions['text_source'] = 'preprocessed_text'
        model_predictions['predicted_label'] = y_pred.map({1: 'POSITIVE', 0: 'NEGATIVE'})
        model_predictions['predicted_positive_probability'] = prob_pos
        ...
        summary_rows.append(
            {
                'model': spec.name,
                'text_source': 'preprocessed_text',
                'rows_scored': len(model_predictions),
                'predicted_negative': predicted_negative,
                'predicted_positive': predicted_positive,
                'predicted_positive_share': predicted_positive / len(model_predictions) if len(model_predictions) else pd.NA,
                'classification_time_seconds': classification_time,
            }
        )
    ...
    prediction_df.to_csv(CORPUS_PREDICTIONS_PATH, index=False)
    summary_df.to_csv(CORPUS_SUMMARY_PATH, index=False)

    print('Corpus predictions saved to:', CORPUS_PREDICTIONS_PATH)
    print('Corpus summary saved to:', CORPUS_SUMMARY_PATH)
    print('\nCorpus class distribution by model:')
    print(summary_df[['model', 'predicted_negative', 'predicted_positive', 'classification_time_seconds']].to_string(index=False))
...
def main() -> None:
    ...
    evaluate_preprocessed_models_on_corpus(preprocessed_artifacts)
    ...
    if os.path.exists(CORPUS_SUMMARY_PATH):
        print('Corpus summary saved to:', CORPUS_SUMMARY_PATH)
    if os.path.exists(CORPUS_PREDICTIONS_PATH):
        print('Corpus predictions saved to:', CORPUS_PREDICTIONS_PATH)
